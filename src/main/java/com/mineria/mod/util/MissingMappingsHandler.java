...
public class MissingMappingsHandler
{
    ...
    private static final ImmutableMap<IForgeRegistry<?>, ImmutableMap<ResourceLocation, ResourceLocation>> MAPPINGS_MAP = ImmutableMap.<IForgeRegistry<?>, ImmutableMap<ResourceLocation, ResourceLocation>>builder()
    ...
    public static void fixMissingMappings(MissingMappings<?> event)
    {
        ...
        List<Mapping<?>> modMappings = event.getAllMappings().stream().filter(mapping -> mapping.key.getNamespace().equals("mineria")).collect(Collectors.toList());
