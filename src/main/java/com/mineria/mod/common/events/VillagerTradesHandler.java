...
{
    private static final Supplier<Int2ObjectMap<List<VillagerTrades.ITrade>>> APOTHECARY_TRADES = () -> Util.make(new Int2ObjectOpenHashMap<>(), map -> {
        map.put(1, ImmutableList.of(
            ...
            new BasicTrade(new ItemStack(Items.EMERALD, 2), Util.make(new ItemStack(Items.SUSPICIOUS_STEW), stack -> SuspiciousStewItem.saveMobEffect(stack, MineriaUtils.getRandomElement(ForgeRegistries.POTIONS.getValues()), 200)), 12, 2, 0.2F),
    ...
    public static final Supplier<ImmutableList<VillagerTrades.ITrade>> ADDITIONAL_CARTOGRAPHER_TRADES = () -> ImmutableList.of(
        new MapTrade(new ItemStack(Items.EMERALD, 17), new ItemStack(Items.COMPASS), MineriaStructures.RITUAL_STRUCTURE.get(), MapDecoration.Type.TARGET_X, 12, 25, 0.2F)
    );
