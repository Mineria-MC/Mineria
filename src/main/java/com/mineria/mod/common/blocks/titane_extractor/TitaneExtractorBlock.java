...
public class TitaneExtractorBlock extends Block
{
    ...
    {
        super(AbstractBlock.Properties.of(Material.METAL).sound(SoundType.METAL).strength(5.0F, 10.0F).harvestTool(ToolType.PICKAXE).requiresCorrectToolForDrops());
    ...
    {
        return MineriaTileEntities.TITANE_EXTRACTOR.get().create();
    }
    ...
    public void onRemove(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving)
    {
        ...
        if(tile instanceof TitaneExtractorTileEntity && state.getBlock() != newState.getBlock())
            InventoryHelper.dropContents(worldIn, pos, ((TitaneExtractorTileEntity)tile).getInventory().toNonNullList());
