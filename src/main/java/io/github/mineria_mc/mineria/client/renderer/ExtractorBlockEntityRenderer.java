package io.github.mineria_mc.mineria.client.renderer;
...
public class ExtractorBlockEntityRenderer implements BlockEntityRenderer<ExtractorBlockEntity> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(Constants.MODID, "textures/block/extractor_gear.png");
    public static final ModelLayerLocation LAYER = new ModelLayerLocation(new ResourceLocation(Constants.MODID, "extractor_gear"), "main");
    private final ExtractorGearModel gearModel;

    public ExtractorBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {
        this.gearModel = new ExtractorGearModel(ctx.bakeLayer(LAYER));
    }

    @Override
    public void render(ExtractorBlockEntity tileEntity, float p_225616_2_, PoseStack stack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        stack.pushPose();
        try {
